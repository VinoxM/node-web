...
export default {
    ...
    methods: {
        getTorrentFromQuery() {
            ...
            const { xt, dn, tr, xs, ws } = route.query;
            if (!xt) {
                return "";
            }
            let result = `magnet:?xt=${xt}`;
            if (dn) {
                result += `&dn=${dn}`;
            } else {
                result += '&dn=Sintel'
            ...
            if (tr) {
                let trArr = Array.isArray(tr) ? tr : [tr];
                result += `&tr=${trArr.join("&tr=")}`;
                result += `&tr=${trackers.join("&tr=")}`;
            }
            if (ws) {
                let wsArr = Array.isArray(ws) ? ws : [ws];
                result += `&ws=${wsArr.join("&ws=")}`;
            // } else {
            //     result += '&ws=https://webtorrent.io/torrents/'
            }
            if (xs) {
                let xsArr = Array.isArray(xs) ? xs : [xs];
                result += `&xs=${xsArr.join("&xs=")}`;
                // } else {
                // result += '&xs=https://webtorrent.io/torrents/sintel.torrent'
            }
            return result;
        },
    ...
    async created() {
        const torrentId = this.getTorrentFromQuery();
        if ((torrentId + "") === "") {
            console.log("No torrent found in query param");
        } else if (Array.isArray(torrentId)) {
            this.addTorrent(torrentId[0])
        } else {
